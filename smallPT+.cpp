...
#define _USE_MATH_DEFINES // 使用数学宏定义 
#include <cmath>
#include <cstdlib>
#include <cstdio>
#include <omp.h>
...
Vec radiance(const Ray& r, int depth, unsigned short* Xi,int E=1) {
    ...
    if (fanshe <= obj.ref) {
		...
		*/
        ...
        double nc = ncg, nt = obj.refr_nt, nnt = into ? nc / nt : nt / nc, ddn = r.d.dot(nl), cos2t;
		/*
		nc为空气的折射率,nt为玻璃的折射
		...
		*/
        ...
        if ((cos2t = 1 - nnt * nnt * (1 - ddn * ddn)) < 0)
			/*
			全反射,当光线从较高 折射率 的 介质 进入到较低折射率的介质时,
			如果入射角大于某一临界角θc(光线远离 法线 )时,折射角将变得足够大,折射后的光线将不会离开介质
			*/
